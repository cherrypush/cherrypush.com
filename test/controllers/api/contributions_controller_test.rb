...
class Api::ContributionsControllerTest < ActionDispatch::IntegrationTest
  ...
  let!(:project) { create(:project, name: 'cherrypush/cherry-cli', user: user) }
  ...
  describe '#create' do
    it 'blocks requests without an api key' do
      post(api_contributions_path, params: deprecated_payload, as: :json)
      assert_response :bad_request
    end

    it 'creates contributions with deprecated payload' do
      post(api_contributions_path, params: { api_key: user.api_key, **deprecated_payload }, as: :json)
      assert_response :ok
      assert_equal 'cherrypush/cherry-cli', Project.sole.name
    ...
    it 'creates contributions' do
      ...
      assert_equal ['dea2fe473f86df94d1103e3c20e5cbdb3f18aad9'], Contribution.all.map(&:commit_sha).uniq
      assert_equal ['2023-02-07T21:33:15.000Z'], Contribution.all.map(&:commit_date).uniq
      assert_equal ['https://github.com/cherrypush/cherry-cli/commit/dea2fe473f86df94d1103e3c20e5cbdb3f18aad9'],
                   Contribution.all.map(&:commit_url).uniq
    end

    it 'updates previous data if pushed twice' do
      post(api_contributions_path, params: { api_key: user.api_key, **deprecated_payload(js_diff: -20, ts_diff: +22) },
                                   as: :json)
      assert_response :ok
      assert_equal [-20, +22], Contribution.all.map(&:diff).sort
      post(api_contributions_path, params: { api_key: user.api_key, **deprecated_payload(js_diff: -30, ts_diff: +33) },
                                   as: :json)
  ...
  def payload(js_diff: -12, ts_diff: +14) # rubocop:disable Metrics/MethodLength
    {
      ...
      commit_sha: 'dea2fe473f86df94d1103e3c20e5cbdb3f18aad9',
      commit_date: '2023-02-07T21:33:15.000Z',
      commit_url: 'https://github.com/cherrypush/cherry-cli/commit/dea2fe473f86df94d1103e3c20e5cbdb3f18aad9',
      contributions: [
        { metric_name: 'JavaScript LoC', diff: js_diff },
        { metric_name: 'TypeScript LoC', diff: ts_diff }
      ]
